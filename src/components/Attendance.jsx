...
const Attendance = () => {
  ...
  const validateAttendance = () => {
    ...
    const [shiftHours, shiftMinutes] = assignedShift.shiftStart.split(":").map(Number);
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log("📌 Assigned Location:", assignedShift.lat, assignedShift.lng);

        const distance = getDistance(userLat, userLng, assignedShift.lat, assignedShift.lng);
        ...
        if (!errors.shift && !errors.location) {
          ...
          setTimeout(() => {
            navigate("/face-scan", { state: { userName, userMobile, userLat, userLng } });
          }, 1000);
        }
      },
      () => {
    ...
    <div className="min-h-screen flex flex-col bg-white">
      <div className="flex-grow flex flex-col md:flex-row items-center justify-center py-12 px-4 md:gap-20">
        <div className="hidden md:block w-1/2">
          <img src="/assets/get-started.svg" alt="Get Started" className="max-w-[80%]" />
        </div>
        ...
        <div className="w-full md:w-1/3 bg-white p-10 shadow-md rounded-lg">
          ...
          <button onClick={validateAttendance} className="w-full bg-orange-500 text-white py-3 rounded-lg hover:bg-orange-600">
            Mark Your Attendance
          </button>

          {error.shift && <p className="text-red-500 text-sm mt-2">{error.shift}</p>}
          {error.location && <p className="text-red-500 text-sm mt-2">{error.location}</p>}
        </div>
      </div>
      ...
      {showLatePopup && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96 text-center">
            <img src="/assets/oh.svg" alt="Oh!" className="mx-auto max-h-20 mb-3" />
            <h2 className="text-red-600 font-bold text-xl">Uh Oh!</h2>
            <p className="text-gray-600 mt-2">You are late today. May we know the reason, please?</p>
            <textarea
