...
const ShiftHandover = () => {
  ...
  const [checklist, setChecklist] = useState([
    { id: 1, label: "Is COMPUTER WORKING OK ?", value: null, remarks: "", image: null },
    { id: 2, label: "Is PRINTER WORKING OK ?", value: null, remarks: "", image: null },
    { id: 3, label: "Are A4 PAGES - 2 BUNDLES in stock ?", value: null, remarks: "", image: null },
    { id: 4, label: "Is OPERATOR TABLE CLEAN and well organised ?", value: null, remarks: "", image: null },
    { id: 5, label: "Is OPERATOR ROOM CLEAN and well organised ?", value: null, remarks: "", image: null },
    { id: 6, label: "Are AREAS AROUND WEIGHBRIDGE clean ?", value: null, remarks: "", image: null },
    { id: 7, label: "Are GITTIS AROUND WEIGHBRIDGE clean ?", value: null, remarks: "", image: null },
    { id: 8, label: "Are there EQUAL WEIGHT ON EACH CORNER of the Weighbridge ?", value: null, remarks: "", image: null },
    { id: 9, label: "Are BOLTS OF WEIGHBRIDGE Tight ?", value: null, remarks: "", image: null },
    { id: 10, label: "Are all CAMERA ANGLES OK ?", value: null, remarks: "", image: null },
    { id: 11, label: "Is SOFTWARE CAPTURING RIGHT IMAGES from the cameras ?", value: null, remarks: "", image: null },
    { id: 12, label: "Is WEIGHBRIDGE PATE free ?", value: null, remarks: "", image: null },
    { id: 13, label: "Have you RANDOMLY PRINTED 15 DAYS OLD SLIP / Parchi ?", value: null, remarks: "", image: null },
  ]);
  ...
  const handleToggle = (id, value) => {
    setChecklist(checklist.map(item => (item.id === id ? { ...item, value } : item)));
  };

  // Function to Handle Remarks Input
  const handleRemarks = (id, text) => {
    setChecklist(checklist.map(item => (item.id === id ? { ...item, remarks: text } : item)));
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white-100 p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <img src="/assets/kanta-king-logo.svg" alt="Kanta King Logo" className="h-15" />
      </div>
      ...
      <p className="text-gray-600 mt-2">📍 Location: {userLat}, {userLng}</p>
      ...
      <div className="bg-white p-4 rounded-lg shadow-lg">
        {checklist.map((item) => (
            ...
            <span className="text-gray-700">
              {item.label.split(/(\b[A-Z\s-]+\b)/).map((part, index) =>
                /^[A-Z\s-]+$/.test(part) ? <strong key={index}>{part}</strong> : part
              )}
            </span>
            ...
            <div className="flex items-center">
              <button
                className={`px-4 py-2 rounded-lg ${
                  item.value === true ? "bg-green-500 text-white" : "bg-gray-300"
                }`}
